...
const HandleGoogleLogin = () => {
  const { googleSignIn, user, loading, setLoading, logOut } = useAuth();
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const dbResponose = await saveUser(result?.user);
      console.log(dbResponose);

      // Get Token
      const token = await getToken(result?.user?.email);
      console.log("token pawa geche", token);
      if (token) {
